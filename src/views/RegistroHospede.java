...
public class RegistroHospede extends JFrame {
	 ...
	 * Create the frame.
	 */
	public RegistroHospede(int idUsuario) {
	...
	public void salvarHospede() {
		try {
			String nome = (( JTextField ) txtNome). getText ();
			String sobrenome = (( JTextField ) txtSobrenome).getText ();
			...
			String telefone = (( JTextField ) txtTelefone). getText ();
			int idReserva = Integer.parseInt(txtNreserva.getText()); 
			if(dataNacimento != null && ! nome.equals("") && ! sobrenome.equals("") && ! telefone.equals("")) {
				Hospede hospedeConstrutor = new Hospede(nome, sobrenome, dataNacimento, nacionalidade, telefone, idReserva );
				this.hospedeController.salvarControoller(hospedeConstrutor);
